...
def check_import(name):
    try:
        ...
        print(
            "Dependancy module '{}' not installed - please install via pip3".format(
                name
            )
        )
        return False
...
class TestResult:
    ...
    def display(self):
        ...
        be_verbose = verbose_test_names is not None and (
            verbose_test_names == [] or self.test_name in verbose_test_names
        )
        ...
        if be_verbose or (status != expected and expected != "wip"):
            ...
            acc += output
            acc += (
                "=" * self.block_len + len("Test Output") * "=" + "=" * self.block_len
            ) + "\n"
    ...
    def validate_config(self):
        ...
        for f in self.settings:
            if not "test_name" in f.keys():
                raise InvalidConfigError(
                    "Test {0} in {1} must contain mandatory field 'test_name', specifying the (unique) name of the test".format(
                        i, self.relpath
                    )
                )
            if not "files" in f.keys():
                raise InvalidConfigError(
                    "Test {0} in {1} must contain mandatory field 'files', a list with at least 1 test".format(
                        i, self.relpath
                    )
                )
            if not "flags" in f.keys():
                raise InvalidConfigError(
                    "Test {0} in {1} must contain mandatory field 'flags', specifying ".format(
                        i, self.relpath
                    )
                )
            if not "expected_result" in f.keys():
                raise InvalidConfigError(
                    "Test {0} in {1} must contain mandatory field 'expected_result'".format(
                        i, self.relpath
                    )
                )
            ...
            if len(f["files"]) == 0:
                raise InvalidConfigError(
                    "Test {0} in {1} must have at least 1 test file".format(
                        i, self.relpath
                    )
                )
            if len(f["flags"]) == 0:
                raise InvalidConfigError(
                    "Test {0} in {1} must have at least 1 compiler flag".format(
                        i, self.relpath
                    )
                )
            if f["expected_result"] not in ["error", "pass", "fail", "wip"]:
                raise InvalidConfigError(
                    """Field 'expected_result' must be one of 'pass', 'fail', 'error' or 'wip' in test {0} in {1}\n. Actual value: {2}""".format(
                        i, self.relpath, str(f["expected_result"])
                    )
                )
            ...
            for k in f.keys():
                if k not in self.valid_test_fields:
                    raise InvalidConfigError(
                        "Field '{0}' not a valid field in test {1} in {2}".format(
                            k, i, self.relpath
                        )
                    )
            ...
            for flag in f["flags"]:
                if re.compile(r"^\s*--dist-dir").match(flag):
                    raise InvalidConfigError(
                        "The use of the '--dist-dir' flag is prohibited in test flags (test {}, in {})".format(
                            i, self.relpath
                        )
                    )
    ...
    def run_cogent(self, filename, flags, test_info):
        ...
        if not os.path.exists(fname):
            ...
            def f():
                return (
                    "error",
                    "Source file '{}' not found".format(fname),
        ...
        def test():
            ...
            res = subprocess.run(
                ["cogent"] + flags + ["--dist-dir={}".format(TEST_DIST_DIR)] + [fname],
...
def run_test(test_name):
    ...
    if conf is None:
        print(
            colored(
                "Cannot find config file containing test name {}".format(test_name),
                "red",
            )
        )
        sys.exit(1)
...
def get_configs_with_errors():
    ...
    for x in files:
        try:
            ...
            print(
                colored(
                    "error - could not find config file for test file {}".format(x),
                    "red",
                )
            )
            errored = True
    ...
    for (n, f) in names:
        ...
        if len(same) > 1 and n not in seen_names:
            ...
            for (x, y) in same:
                ...
                print(
                    "Seen {1} time(s) in {2}".format(
                        x, len(samefile) - len(set(samefile)) + 1, y
                    )
                )
                seen_files.add(y)
        seen_names.add(n)
...
if __name__ == "__main__":
    ...
    if cogent is None:
        print(
            "Could not find cogent compiler on PATH - Please install cogent and place it on the PATH"
        )
        sys.exit(1)
