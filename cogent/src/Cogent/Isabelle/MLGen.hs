...
mkTriple a b c = MLPair a (MLPair b c)
...
deepType (TSum alts)      = mkApp (mkId "TSum"    ) (mkList $ map (\(n,(t,b)) -> mkTriple (mkString n) (deepType t) (mkBool b)) alts)
deepType (TProduct t1 t2) = mkApp (mkId "TProduct") $ mkPair (deepType t1) (deepType t2)
deepType (TRecord fs s)   = mkApp (mkId "TRecord" ) $
                              mkRecord [ ("fields", mkList (map (\(fn,(t,b)) -> mkTriple (mkString fn) (deepType t) (mkBool b)) fs))
                                       , ("sigil", deepSigil s)
                                       ]
...
deepExpr (TE _ (Op op es)       ) = mkApp (mkId "Op") $ mkRecord [ ("oper", deepOp op)
                                                                 , ("exprs", mkList (map deepExpr es))
                                                                 ]
deepExpr (TE _ (App e1 e2)      ) = mkApp (mkId "App") $ mkPair (deepExpr e1) (deepExpr e2)
deepExpr (TE _ (Con n e t)      ) = mkApp (mkId "Con") $ mkRecord [ ("varCase", mkString n)
                                                                  , ("val", deepExpr e)
                                                                  , ("ty", deepType t)
                                                                  ]
deepExpr (TE _ (Unit)          ) = mkId "Unit"
deepExpr (TE _ (ILit i U8 )    ) = mkApp (mkId "LU8" ) $ mkInteger i
deepExpr (TE _ (ILit i U16)    ) = mkApp (mkId "LU16") $ mkInteger i
deepExpr (TE _ (ILit i U32)    ) = mkApp (mkId "LU32") $ mkInteger i
deepExpr (TE _ (ILit i U64)    ) = mkApp (mkId "LU64") $ mkInteger i
-- FIXME: these booleans are not technically the cogent semantics
deepExpr (TE _ (ILit i Boolean)) = mkApp (mkId "LBool") $ mkBool (if i == 0 then False else True)
deepExpr (TE _ (SLit s)        ) = mkApp (mkId "SLit") $ mkString s
deepExpr (TE _ (Let _ ea ek)   ) = mkApp (mkId "Let") $ mkRecord [ ("eval", deepExpr ea)
                                                                 , ("econt", deepExpr ek)
                                                                 ]
deepExpr (TE _ (LetBang bs _ ea ek)) = mkApp (mkId "Let") $ mkRecord [ ("indices", mkList $ map (mkInt . Vec.finInt . fst) bs)
                                                                     , ("eval", deepExpr ea)
                                                                     , ("econt", deepExpr ek)
                                                                     ]
deepExpr (TE _ (Tuple e1 e2)    ) = mkApp (mkId "Tuple") $ mkPair (deepExpr e1) (deepExpr e2)
deepExpr (TE _ (Struct fs)      ) = mkApp (mkId "Struct") $ mkList (map (\(n,e) -> mkPair (mkString n) (deepExpr e)) fs)
...
deepExpr (TE _ (Split (_,_) e1 e2) ) = mkApp (mkId "Split") $ mkPair (deepExpr e1) (deepExpr e2)
deepExpr (TE _ (Member e i)        ) = mkApp (mkId "Member") $ mkPair (deepExpr e) (mkInt i)
deepExpr (TE _ (Take (_,_) ev i ek)) = mkApp (mkId "Take") $ mkRecord [ ("erec", deepExpr ev)
                                                                      , ("idx", mkInt i)
                                                                      , ("ek", deepExpr ek)
                                                                      ]
deepExpr (TE _ (Put er i ev)) = mkApp (mkId "Put") $ mkRecord [ ("erec", deepExpr er)
                                                              , ("idx", mkInt i)
                                                              , ("eval", deepExpr ev)
                                                              ]
